...
class Token:
    ...
    def consVowelsPattern(self):
        conVow_NonC = self.word.lower()
        paterns = [['[bcdfghjklmnpqrstwxyz]', 'C'], ['[aeiou]', 'V']]

        for i in paterns:
            conVow_NonC = re.sub(i[0],i[1],conVow_NonC)
        
        symbols = ['C', 'V']
        conVow_C = conVow_NonC
        for i in symbols:
            conVow_C = re.sub(2*i+"+",2*i,conVow_C)
        return conVow_NonC, conVow_C
    ...
    def extract_features(self, func, mask):
        ...
        if mask[20]:
            if self.word.isupper():
                features.append(func('ALLCAPS'))

        # fist cap feature
        if mask[21]:
            if self.word[0].isupper():
                features.append(func('FIRSTCAP'))

        # two or more caps feature
        if mask[22]:
            if sum(1 for l in self.word if l.isupper())>1:
                features.append(func('MORE2CAP'))

	conVow_NonC, conVow_C = self.consVowelsPattern()
	
	# consonant vowel pattern
	if mask[20]:
            features.append(func('CV_NC:%s' % conVow_NonC))
	    
	# consonant vowel pattern collapsed
        if mask[21]:
            features.append(func('CV_C:%s' % conVow_C))
	    '''
	# previous POS
        if mask[22]:
            features.append(func('PREV_POS:%s' % self.prev_pos()))
